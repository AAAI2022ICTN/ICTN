...
parser.add_argument('-c', '--checkpoint', default='joint_checkpoint', type=str, metavar='PATH',
...
parser.add_argument('--lstm_hid_dim', default=150, type=int, metavar='N',
                    help='lstm_hid_dim')
parser.add_argument('--num_class', default=54, type=int, metavar='N',
                    help='the number of class')
parser.add_argument('--num-sample', default=10, type=int,
...
def main():
    ...
    train_loader, val_loader, embed = data_got.joint_loader(batch_size=args.batch_size)
    # test_loader = data_got.test_loader(batch_size=args.batch_size)
    # Ftest_loader, novel_loader, embed = data_got.Nload_data(batch_size=args.batch_size,sample_num=args.num_sample)
    embed = torch.from_numpy(embed).float()
    if not os.path.isdir(args.checkpoint):
        mkdir_p(args.checkpoint)

    model = models.Net(args.batch_size, args.lstm_hid_dim, args.d_a, args.num_class, embed).cuda()
